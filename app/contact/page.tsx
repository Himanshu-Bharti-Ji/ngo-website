      ...
      <section className="py-14 sm:py-20 bg-[#fdf8f0]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
            ...
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-600 mb-4">Reach us</p>
              <div className="w-10 h-0.5 bg-gradient-to-r from-amber-500 to-amber-300 rounded-full mb-6" />
              <h2 className="text-3xl sm:text-4xl font-bold text-[#1a1208] mb-8" style={{fontFamily:"'Playfair Display', serif"}}>
              ...
              <div className="space-y-4">
                {[
                  { icon: <MapPin size={20} className="text-white"/>, color: "bg-[#0d2b1a]", label: "Address", content: `${ngo.name}\n${address.line1}\n${address.city}` },
                  { icon: <Phone size={20} className="text-white"/>, color: "bg-emerald-600", label: "Phone", content: ngo.contact.phone, href: `tel:${ngo.contact.phone}` },
                  { icon: <Mail size={20} className="text-white"/>, color: "bg-amber-500", label: "Email", content: ngo.contact.email, href: `mailto:${ngo.contact.email}` },
                  { icon: <Clock size={20} className="text-white"/>, color: "bg-[#1a5c3a]", label: "Hours", content: `${officeHours.weekdays}\n${officeHours.saturday}\n${officeHours.sunday}` },
                ].map((item, i) => (
                  <div key={i} className="flex items-start gap-4 p-5 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div>
                      ...
                      {item.href ? (
                        <a href={item.href} className="text-sm sm:text-base text-[#1a1208] hover:text-emerald-700 font-medium transition-colors">
                          {item.content}
                        </a>
                      ) : (
                        <p className="text-sm sm:text-base text-[#1a1208] font-medium whitespace-pre-line">{item.content}</p>
                      )}
                    </div>
                  </div>
                ))}
            ...
            <div>
              <div className="bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-gray-100 p-7 sm:p-9">
                <h2 className="text-2xl sm:text-3xl font-bold text-[#1a1208] mb-2" style={{fontFamily:"'Playfair Display', serif"}}>
                  Send us a message
                </h2>
