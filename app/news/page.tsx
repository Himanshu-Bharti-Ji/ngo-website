  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="group relative overflow-hidden rounded-2xl sm:rounded-3xl mb-6 sm:mb-8 cursor-pointer">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-10">
              <div className="flex items-center gap-3 mb-3 flex-wrap">
                <span
                  className={`text-xs font-bold text-white rounded-full px-3 py-1 ${categoryColors[featured.category] || "bg-emerald-500"}`}
                >
                  {featured.category}
                </span>
              ...
              <h2
                className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-3 max-w-2xl"
                style={{ fontFamily: "'Playfair Display', serif" }}
              >
                {featured.title}
              </h2>
              <p className="text-slate-300 text-sm sm:text-base mb-5 max-w-xl line-clamp-2">
                {featured.description}
              </p>
              <a
                href="#"
                className="inline-flex items-center gap-2 px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-white font-bold rounded-full text-sm transition-all"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {rest.map((item, i) => (
              <div
                key={i}
                className="group bg-white rounded-2xl sm:rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow cursor-pointer"
              >
                <div className="relative h-44 sm:h-48 overflow-hidden">
                  ...
                  <span
                    className={`absolute top-4 left-4 text-xs font-bold text-white rounded-full px-3 py-1 ${categoryColors[item.category] || "bg-emerald-500"}`}
                  >
                    {item.category}
                  </span>
                </div>
                <div className="p-5 sm:p-6">
                  ...
                  <h3
                    className="font-bold text-[#1a1208] text-base sm:text-lg leading-snug mb-4 group-hover:text-emerald-700 transition-colors"
                    style={{ fontFamily: "'Playfair Display', serif" }}
                  >
                    {item.title}
                  </h3>
      ...
      <section
        ...
        style={{
          backgroundImage:
            "url('https://images.pexels.com/photos/6646907/pexels-photo-6646907.jpeg?auto=compress&cs=tinysrgb&w=1920&h=600&fit=crop')",
      ...
      >
        ...
        <div className="relative max-w-2xl mx-auto px-4 sm:px-6 py-16 sm:py-20 text-center">
          ...
          <p className="text-slate-300 mb-8 text-sm sm:text-base">
            Subscribe to our newsletter — field stories, impact reports, and
            program updates delivered to you.
          </p>
          <form className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input
              type="email"
              placeholder="Enter your email"
              className="flex-1 px-5 py-3 rounded-full bg-white/10 border border-white/20 text-white placeholder-slate-400 text-sm focus:outline-none focus:border-emerald-400 transition-colors"
              required
            />
            <button
              type="submit"
